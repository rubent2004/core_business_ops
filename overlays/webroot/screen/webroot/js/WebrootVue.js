...
window.cfMenuGroupMap = window.cfMenuGroupMap || {
    Catalog: 'business',
    Customer: 'business',
    Party: 'business',
    Supplier: 'business',
    ...
    HumanRes: 'business',
    Accounting: 'business',
    ProductStore: 'business',
    Project: 'business',
    Task: 'business',
    Request: 'business',
    Survey: 'business',
    Wiki: 'business',
    QuickSearch: 'business',
    QuickViewReport: 'business',
    SimpleReport: 'business',
    Manufacturing: 'ops',
    Facility: 'ops',
    Order: 'ops',
    Shipment: 'ops',
    Shipping: 'ops',
    Return: 'ops',
    SvLocalization: 'fiscal',
    dashboard: 'home'
};
window.cfGroupOrder = window.cfGroupOrder || ['home', 'business', 'ops', 'fiscal', 'other'];
window.cfGroupLabels = window.cfGroupLabels || {
    home: 'INICIO',
...
moqui.webrootVue = new Vue({
    ...
    methods: {
        ...
        /* Agrupa el segundo nivel del navMenuList del sidebar en CORE BUSINESS /
           CORE OPS / FISCAL SV. El mapping nombre→grupo se inyecta desde el FTL
           en window.cfMenuGroupMap; los que no matchean caen en "other". */
        cfGroupedSubscreens: function(subscreens) {
            var map = window.cfMenuGroupMap || {};
            var order = window.cfGroupOrder || ['business','ops','fiscal','other'];
            var labels = window.cfGroupLabels || {};
            var buckets = {};
            order.forEach(function(id) { buckets[id] = []; });
            (subscreens || []).forEach(function(sub) {
                var key = sub.name || sub.title || '';
                var groupId = map[key] || 'other';
                if (!buckets[groupId]) buckets[groupId] = [];
                buckets[groupId].push(sub);
            });
            return order.map(function(id) {
                return { id: id, label: labels[id] || id.toUpperCase(), items: buckets[id] || [] };
            });
        }
    },
    watch: {
